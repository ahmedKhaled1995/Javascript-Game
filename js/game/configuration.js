const FPS = 60;
const TIME_FOR_DIFFICULTY_TO_SPIKE = 5;

const GAME_WORLD_WIDTH = 960;
const GAME_WORLD_HEIGHT = 540;

const PLAYER_STARTING_X = 50;
const PLAYER_STARTING_Y = GAME_WORLD_HEIGHT / 2;
const PLAYER_SPEED = 3;
const PLAYER_WIDTH = 50;
const PLAYER_HEIGHT = 75;
const PLAYER_PROJECTILE_SPEED = 3;
const PLAYER_PROJECTILE_COOLDOWNTIME = .7; 
const PLAYER_LIFES = 3;
const PLAYER_SCORE_BONUS = 5;
const PLAYER_TAKEN_DAMAGE = 1; 

const OBSTCALE_SPEED = 1;
const OBSTCALE_GENERATION_SPEED = 1000;
const OBSTCALE_COLOR = "crimson";
const OBSTCALE_STARTING_X = 300;
const OBSTCALE_WIDTH = 18;
const OBSTCALE_MAX_HEIGHT = (GAME_WORLD_HEIGHT - PLAYER_HEIGHT) / 2;
const OBSTCALE_MIN_HEIGHT = 20;
const OBSTCALE_VERTICAL_INCREMENT = 3;
const LOWER_OBSTCALE_STARTING_POINT = GAME_WORLD_HEIGHT - OBSTCALE_MAX_HEIGHT;
const HEIGHER_OBSTCALE_STARTING_POINT = 0;
const SAFE_FACTOR = 12;

const PROJECTILE_SPEED = 1;
const PROJECTILE_GENERATION_SPEED = 2000;
const PROJECTILE_WIDTH = 30;
const PROJECTILE_HEIGHT = 30;

const GAME_CONFIG = {
    FPS,
    TIME_FOR_DIFFICULTY_TO_SPIKE,
    GAME_WORLD_WIDTH,
    GAME_WORLD_HEIGHT,
    PLAYER_STARTING_X,
    PLAYER_STARTING_Y,
    PLAYER_SPEED,
    PLAYER_WIDTH,
    PLAYER_HEIGHT,
    PLAYER_PROJECTILE_SPEED,
    PLAYER_PROJECTILE_COOLDOWNTIME,
    PLAYER_LIFES,
    PLAYER_SCORE_BONUS,
    PLAYER_TAKEN_DAMAGE,
    OBSTCALE_SPEED,
    OBSTCALE_GENERATION_SPEED,
    OBSTCALE_COLOR,
    OBSTCALE_STARTING_X,
    OBSTCALE_WIDTH,
    OBSTCALE_MAX_HEIGHT,
    OBSTCALE_MIN_HEIGHT,
    OBSTCALE_VERTICAL_INCREMENT,
    LOWER_OBSTCALE_STARTING_POINT,
    HEIGHER_OBSTCALE_STARTING_POINT,
    SAFE_FACTOR,
    PROJECTILE_SPEED,
    PROJECTILE_GENERATION_SPEED,
    PROJECTILE_WIDTH,
    PROJECTILE_HEIGHT,
};

export default GAME_CONFIG;